...
# link: <https://github.com/Pyrax/samp-docker>
# copyright: Bjorn Clees - 2017
...
if [ "$1" = 'samp' ]; then
    ...
    for config in $*; do
        ...
        grep -q "$key" /samp03/server.cfg && \
            sed -i "s/$key .*/$key $value/" /samp03/server.cfg 2>/dev/null || \
            sed -i "$ a\\$key $value" /samp03/server.cfg 2>/dev/null
    done
